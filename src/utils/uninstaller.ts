...
export class Uninstaller {
   ...
   */
  async findAIAssistantFiles(): Promise<{ files: (string | { path: string, aiSettings: Record<string, any> })[], directories: string[] }> {
    const aiFiles: (string | { path: string, aiSettings: Record<string, any> })[] = [];
  ...
  async uninstall(options: UninstallOptions = {}): Promise<UninstallResult> {
    ...
    if (dryRun) {
      Feedback.info('Running in dry-run mode. No files will actually be removed.');
      Feedback.info('This will show you what would be removed if you run without --dry-run.');
    }
    ...
    try {
      ...
      Feedback.info(`Found ${files.length} files and ${directories.length} directories to ${dryRun ? 'potentially ' : ''}remove.`);
        ...
        if (await fs.pathExists(vscodeSettingsPath)) {
          try {
            ...
            if (Object.keys(removedSettings).length > 0 && !dryRun) {
              ...
              Feedback.success(`Removed ${Object.keys(removedSettings).length} AI assistant settings from VS Code settings.json`);
            } else if (Object.keys(removedSettings).length > 0) {
              Feedback.info(`Would remove ${Object.keys(removedSettings).length} AI assistant settings from VS Code settings.json`);
            }
          } catch (error) {
            Feedback.warning(`Failed to update VS Code settings: ${error instanceof Error ? error.message : String(error)}`);
          }
        }
        
        // Check and clean extensions.json
        const extensionsPath = path.join(this.projectRoot, '.vscode', 'extensions.json');
        if (await fs.pathExists(extensionsPath)) {
          try {
            ...
            if (extensions.recommendations) {
              ...
              extensions.recommendations = extensions.recommendations.filter((ext: string) => !aiExtensions.includes(ext));
            ...
            if (extensions.unwantedRecommendations) {
              ...
              extensions.unwantedRecommendations = extensions.unwantedRecommendations.filter((ext: string) => !copilotExtensions.includes(ext));
              changed = changed || originalLength !== extensions.unwantedRecommendations.length;
            }
            ...
            Feedback.warning(`Failed to update VS Code extensions.json: ${error instanceof Error ? error.message : String(error)}`);
          }
        }
      }
      ...
      if (await fs.pathExists(envPath)) {
        try {
          ...
          if (removedLines.length > 0 && !dryRun) {
            ...
            Feedback.success(`Removed ${removedLines.length} AI assistant environment variables from .env`);
          } else if (removedLines.length > 0) {
            Feedback.info(`Would remove ${removedLines.length} AI assistant environment variables from .env`);
          }
        } catch (error) {
          Feedback.warning(`Failed to update .env file: ${error instanceof Error ? error.message : String(error)}`);
        }
      }
      
      // Check for Dependabot configuration file
      const dependabotPath = path.join(this.projectRoot, '.github', 'dependabot.yml');
      if (await fs.pathExists(dependabotPath)) {
        if (dryRun) {
          Feedback.info(`Would remove Dependabot configuration: ${dependabotPath}`);
        } else {
          await fs.remove(dependabotPath);
          Feedback.success(`Removed Dependabot configuration: ${dependabotPath}`);
        }
        ...
        try {
          ...
          if (githubDirContents.length === 0) {
            if (dryRun) {
              Feedback.info(`Would remove empty .github directory: ${githubDirPath}`);
            } else {
              await fs.remove(githubDirPath);
              Feedback.success(`Removed empty .github directory: ${githubDirPath}`);
            }
          }
        } catch (error) {
      ...
      if (dryRun) {
        ...
        Feedback.info('To perform the actual removal, run the command without the --dry-run flag.');
      } else {
        Feedback.success('All AI coding assistant configuration files have been removed.');
      }
      
      if (!includeVSCode) {
        Feedback.info('Note: VS Code settings were not modified to avoid disrupting your editor configuration.');
        Feedback.info('If desired, you can manually remove AI assistant settings from your VS Code settings.json.');
        Feedback.info('To include VS Code settings in the removal, use the --include-vscode flag.');
      }
      ...
      Feedback.error(`Error during uninstall: ${error instanceof Error ? error.message : String(error)}`);
      return { 
        success: false, 
        removed: { 
          files: [], 
