...
export default function () {
  ...
  group('Concurrent API Calls', () => {
    const batch = http.batch([
      ['GET', `${BASE_URL}/api/items`, null, { headers, tags: { name: 'BatchGet1' } }],
      ['GET', `${BASE_URL}/api/items?page=2`, null, { headers, tags: { name: 'BatchGet2' } }],
      ['GET', `${BASE_URL}/api/status`, null, { headers, tags: { name: 'BatchStatus' } }],
    ]);
  ...
  group('Resource Intensive Operations', () => {
    ...
    const heavyResponse = http.post(`${BASE_URL}/api/bulk-process`, largePayload, {
      headers,
  ...
  group('Rapid Requests', () => {
    for (let i = 0; i < 5; i++) {
      const rapidResponse = http.get(`${BASE_URL}/api/items/${Math.floor(Math.random() * 100)}`, {
        headers,
        tags: { name: 'RapidFire' },
      });
...
export function handleSummary(data) {
  ...
  const summary = {
    ...
    custom_metrics: {
      ...
      avg_custom_response_time: Math.round(metrics.custom_response_time?.values?.avg || 0),
    },
