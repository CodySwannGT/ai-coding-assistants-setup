...
class PreRebaseHook extends BaseHook {
  constructor(config) {
    ...
    this.checkConflicts = config.checkConflicts !== false; // Whether to check for potential merge conflicts
    this.checkTestImpact = config.checkTestImpact || false; // Whether to check for impact on tests
    this.checkDependencies = config.checkDependencies || true; // Whether to check for dependency changes
...
\`\`\``;
  ...
  async analyzeRebase(diff, commitsToRebase, affectedFiles, upstreamBranch, rebaseBranch) {
    ...
    if (!this.claudeApiKey) {
      this.warn('No Claude API key found, skipping detailed analysis');
      
      // Return a basic analysis based on the affected files
      return {
        issues: affectedFiles.common.length > 0 ? [
          {
            severity: 'medium',
            description: `${affectedFiles.common.length} files have been modified in both branches and may have conflicts`
          }
        ] : [],
        summary: `Basic analysis without Claude API:
...
- Files modified in both branches: ${affectedFiles.common.length}`
    ...
    try {
      ...
      return {
        ...
        summary: `Basic analysis (Claude API call failed):
- Files modified in ${rebaseBranch}: ${affectedFiles.rebase.length}
- Files modified in ${upstreamBranch}: ${affectedFiles.upstream.length}
- Files modified in both branches: ${affectedFiles.common.length}`
      };
