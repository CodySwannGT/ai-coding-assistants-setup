...
import { ProjectDetector, PackageManager } from '../../src/utils/project-detector';
...
describe('ProjectDetector', () => {
  ...
  describe('detectPackageManager', () => {
    test('should detect yarn from yarn.lock', async () => {
      ...
      // Execute
      const result = await ProjectDetector.detectPackageManager(testProjectPath);
  ...
  describe('detectFeatures', () => {
    ...
    test('should handle errors reading package.json', async () => {
      ...
      (fs.readJson as jest.Mock).mockRejectedValue(new Error('Failed to read file'));
  ...
  describe('getInstallCommand', () => {
    test('should get correct install command for each package manager', () => {
      expect(ProjectDetector.getInstallCommand(PackageManager.NPM)).toBe('npm install');
      expect(ProjectDetector.getInstallCommand(PackageManager.YARN)).toBe('yarn');
      expect(ProjectDetector.getInstallCommand(PackageManager.PNPM)).toBe('pnpm install');
      expect(ProjectDetector.getInstallCommand(PackageManager.BUN)).toBe('bun install');
      expect(ProjectDetector.getInstallCommand(PackageManager.UNKNOWN)).toBe('npm install');
    });
  });
  ...
  describe('getRunCommand', () => {
    test('should get correct run command for each package manager', () => {
      ...
      expect(ProjectDetector.getRunCommand(PackageManager.NPM, scriptName)).toBe('npm run test');
      expect(ProjectDetector.getRunCommand(PackageManager.YARN, scriptName)).toBe('yarn test');
      expect(ProjectDetector.getRunCommand(PackageManager.PNPM, scriptName)).toBe('pnpm run test');
      expect(ProjectDetector.getRunCommand(PackageManager.BUN, scriptName)).toBe('bun run test');
      expect(ProjectDetector.getRunCommand(PackageManager.UNKNOWN, scriptName)).toBe('npm run test');
    });
  });
