...
export class CommitLintSetup {
  ...
  async installPackages(installCommitizen: boolean = false): Promise<boolean> {
    try {
      ...
      Feedback.error(
        `Failed to install commitlint packages: ${error instanceof Error ? error.message : String(error)}`
      );
      return false;
    }
  }
  ...
  async createConfig(): Promise<boolean> {
    try {
      ...
      if (await fs.pathExists(configPath)) {
        Feedback.warning(
          'commitlint.config.js already exists. Skipping creation.'
        );
        return true;
      }
  ...
  rules: {
    ...
    'subject-case': [
      ...
      Feedback.error(
        `Failed to create commitlint configuration: ${error instanceof Error ? error.message : String(error)}`
      );
      return false;
    }
  }
  ...
  async createCommitMsgTemplate(): Promise<boolean> {
    try {
      ...
      if (await fs.pathExists(templatePath)) {
        Feedback.warning(
          '.gitmessage template already exists. Skipping creation.'
        );
        return true;
      }
...
`;
      ...
      Feedback.error(
        `Failed to create commit message template: ${error instanceof Error ? error.message : String(error)}`
      );
      return false;
    }
  }
...
`;
      ...
      Feedback.error(
        `Failed to set up husky hook: ${error instanceof Error ? error.message : String(error)}`
      );
      return false;
    }
  }
  ...
  async setupCommitizen(): Promise<boolean> {
    try {
      ...
      Feedback.error(
        `Failed to set up commitizen: ${error instanceof Error ? error.message : String(error)}`
      );
      return false;
    }
  }
  ...
  async setupCommitLint(options: CommitLintOptions = {}): Promise<boolean> {
    const { setupHusky = true, installCommitizen = false, cwd } = options;
