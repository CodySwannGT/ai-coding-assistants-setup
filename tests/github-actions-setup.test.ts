...
describe('GitHubActionsSetup', () => {
  ...
  describe('ensureDirectories', () => {
    it('should create GitHub workflows directories', async () => {
      ...
      expect(mockedFs.ensureDir).toHaveBeenCalledWith(path.join(testDir, '.github'));
      expect(mockedFs.ensureDir).toHaveBeenCalledWith(path.join(testDir, '.github', 'workflows'));
    });
    
    it('should handle errors', async () => {
      mockedFs.ensureDir.mockRejectedValueOnce(new Error('Directory creation failed'));
  ...
  describe('setupPackageJsonScripts', () => {
    it('should add missing CI scripts to package.json', async () => {
      ...
      expect(mockedFs.readJson).toHaveBeenCalledWith(path.join(testDir, 'package.json'));
  ...
  describe('setupGitHubActions', () => {
    it('should set up all workflows by default', async () => {
      ...
      const spyCIWorkflow = jest.spyOn(setup, 'setupCIWorkflow').mockResolvedValue(true);
      const spyReleaseWorkflow = jest.spyOn(setup, 'setupReleaseWorkflow').mockResolvedValue(true);
      const spyDocsWorkflow = jest.spyOn(setup, 'setupDocsWorkflow').mockResolvedValue(true);
      const spyIssueWorkflow = jest.spyOn(setup, 'setupIssueOnFailureWorkflow').mockResolvedValue(true);
      const spyDependabot = jest.spyOn(setup, 'setupDependabot').mockResolvedValue(true);
      const spyScripts = jest.spyOn(setup, 'setupPackageJsonScripts').mockResolvedValue(true);
    ...
    it('should handle directory creation failure', async () => {
      ...
      const spyDirs = jest.spyOn(setup, 'ensureDirectories').mockResolvedValue(false);
