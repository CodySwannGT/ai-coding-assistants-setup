...
export class PrettierSetup {
  ...
  async installPrettier(): Promise<boolean> {
    try {
      ...
      let dependencies: any = {
        prettier: DEFAULT_PRETTIER_DEPENDENCIES.prettier,
      };
      ...
      if (hasEslint) {
        dependencies = {
          ...
          'eslint-config-prettier':
            DEFAULT_PRETTIER_DEPENDENCIES['eslint-config-prettier'],
          'eslint-plugin-prettier':
            DEFAULT_PRETTIER_DEPENDENCIES['eslint-plugin-prettier'],
        };
      }
      ...
      Feedback.error(
        `Failed to install Prettier: ${error instanceof Error ? error.message : String(error)}`
      );
      return false;
    }
  }
  ...
  async createPrettierConfig(): Promise<boolean> {
    try {
      ...
      Feedback.error(
        `Failed to create Prettier config: ${error instanceof Error ? error.message : String(error)}`
      );
      return false;
    }
  }
  ...
  async createPrettierIgnore(): Promise<boolean> {
    try {
      ...
      Feedback.error(
        `Failed to create .prettierignore: ${error instanceof Error ? error.message : String(error)}`
      );
      return false;
    }
  }
  ...
  async addPrettierScripts(): Promise<boolean> {
    try {
      ...
      Feedback.error(
        `Failed to add Prettier scripts: ${error instanceof Error ? error.message : String(error)}`
      );
      return false;
    }
  }
  ...
  async integratePrettierWithESLint(): Promise<boolean> {
    try {
      ...
      if (!(await fs.pathExists(eslintConfigPath))) {
        Feedback.warning(
          'ESLint configuration not found, skipping integration'
        );
        return false;
      }
      ...
      if (
        !eslintConfig.extends.includes('prettier') &&
        !eslintConfig.extends.includes('plugin:prettier/recommended')
      ) {
      ...
      Feedback.error(
        `Failed to integrate Prettier with ESLint: ${error instanceof Error ? error.message : String(error)}`
      );
      return false;
    }
  }
