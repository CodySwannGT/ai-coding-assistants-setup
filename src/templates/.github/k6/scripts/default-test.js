...
if (__ENV.K6_DURATION && scenarios[SCENARIO]) {
  if (scenarios[SCENARIO].duration) {
    ...
    scenarios[SCENARIO].stages = scenarios[SCENARIO].stages.map(stage => ({
      ...stage,
      duration: Math.ceil(parseInt(stage.duration) * scale) + stage.duration.slice(-1),
    }));
  }
}
...
export default function () {
  ...
  endpoints.forEach(endpoint => {
    ...
    const response = http.request(endpoint.method, url, null, { headers, tags: { name: endpoint.name } });
...
export function setup() {
  ...
  if (response.status !== 200 && response.status !== 301 && response.status !== 302) {
    throw new Error(`Base URL ${BASE_URL} is not accessible. Status: ${response.status}`);
  }
