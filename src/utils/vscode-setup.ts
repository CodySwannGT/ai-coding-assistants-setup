...
export class VSCodeSetup {
   ...
   */
  async detectGitHubCopilot(): Promise<{
    installed: boolean;
    path: string | null;
  }> {
    ...
    try {
        ...
        try {
          if (await fs.pathExists(extPath)) {
            ...
            if (copilotMatches.length > 0) {
              ...
              Feedback.info(
                `Found GitHub Copilot at integrated location: ${copilotPath}`
              );
              return { installed: true, path: copilotPath };
            }
          }
      ...
      Feedback.warning(
        `Error detecting GitHub Copilot: ${error instanceof Error ? error.message : String(error)}`
      );
      return { installed: false, path: null };
    }
  }
  ...
  private getVSCodeUserConfigPath(): string | null {
    try {
      ...
      Feedback.warning(
        `Error getting VS Code config path: ${error instanceof Error ? error.message : String(error)}`
      );
      return null;
    }
  }
  ...
  private getVSCodeExtensionsPath(): string | null {
    try {
      ...
      Feedback.warning(
        `Error getting VS Code extensions path: ${error instanceof Error ? error.message : String(error)}`
      );
      return null;
    }
  }
  ...
  async createVSCodeDirectories(dryRun: boolean = false): Promise<boolean> {
    try {
      ...
      Feedback.error(
        `Failed to create VS Code directories: ${error instanceof Error ? error.message : String(error)}`
      );
      return false;
    }
  }
  ...
  async updateSettings(
    ...
    try {
      ...
      const { installed: copilotInstalled, path: copilotPath } =
        await this.detectGitHubCopilot();
      ...
      if (copilotInstalled) {
        ...
        if (disableCopilot) {
          Feedback.info(
            'Configuring settings to disable GitHub Copilot for this workspace'
          );
          ...
          Feedback.info(
            'GitHub Copilot will remain active alongside Claude/Roo - be aware of potential conflicts'
          );
          ...
          if (prioritizeClaudeRoo) {
            ...
            Feedback.success(
              'Settings configured to prioritize Claude/Roo over GitHub Copilot'
            );
          }
        }
      } else {
   ...
   */
  async updateExtensionsRecommendations(
    dryRun: boolean = false
  ): Promise<boolean> {
    try {
      // Create VS Code directories if they don't exist
      ...
      if (copilotInstalled) {
        ...
        if (
          !extensions.unwantedRecommendations.includes('github.copilot-chat')
        ) {
      ...
      Feedback.error(
        `Failed to update VS Code extensions recommendations: ${error instanceof Error ? error.message : String(error)}`
      );
      return false;
    }
  }
  ...
  async setup(
    ...
    try {
      ...
      const extensionsUpdated =
        await this.updateExtensionsRecommendations(dryRun);
      ...
      Feedback.error(
        `VS Code setup failed: ${error instanceof Error ? error.message : String(error)}`
      );
      return false;
    }
  }
