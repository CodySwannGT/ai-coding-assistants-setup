...
export class ConfigMerger {
  ...
  static parseConfigFile(filePath: string, fileType: ConfigFileType): any {
    try {
      ...
      Feedback.error(`Failed to parse config file ${filePath}: ${error instanceof Error ? error.message : String(error)}`);
      return null;
    }
  }
  ...
  static stringifyConfig(config: any, fileType: ConfigFileType): string {
    try {
      ...
      throw new Error(`Failed to stringify config: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
   ...
   */
  private static async promptForStrategy(filePath: string): Promise<ConflictStrategy> {
    ...
    try {
      ...
      if (!fs.existsSync(targetPath)) {
        try {
          ...
          Feedback.error(`Failed to copy configuration file to ${targetPath}: ${error instanceof Error ? error.message : String(error)}`);
          return false;
        }
      }
      ...
      if (fileType === ConfigFileType.UNKNOWN) {
        if (verbose) {
          Feedback.info(`File type unknown, using standard file merger for: ${path.basename(sourcePath)}`);
        }
        return FileMerger.mergeFile(sourcePath, targetPath, defaultMergeOption, interactive);
      }
      ...
      if (sourceConfig === null || targetConfig === null) {
        if (verbose) {
          Feedback.info(`Failed to parse config files, using standard file merger for: ${path.basename(sourcePath)}`);
        }
        return FileMerger.mergeFile(sourcePath, targetPath, defaultMergeOption, interactive);
      }
      ...
      switch (strategy) {
        case ConflictStrategy.USE_SOURCE:
          ...
          Feedback.info(`Using source config for: ${path.basename(targetPath)}`);
          break;
          
        case ConflictStrategy.USE_TARGET:
          resultConfig = targetConfig;
          Feedback.info(`Keeping target config for: ${path.basename(targetPath)}`);
          break;
          
        case ConflictStrategy.MERGE:
          resultConfig = this.deepMerge(targetConfig, sourceConfig);
          Feedback.info(`Merged configurations for: ${path.basename(targetPath)}`);
          break;
          
        case ConflictStrategy.MANUAL:
          // TODO: Implement editor opening for manual resolution
          Feedback.warning(`Manual resolution not yet implemented. Using merge strategy for: ${path.basename(targetPath)}`);
          resultConfig = this.deepMerge(targetConfig, sourceConfig);
          break;
      }
      ...
      Feedback.success(`Successfully merged configuration file: ${path.basename(targetPath)}`);
      return true;
    } catch (error) {
      Feedback.error(`Error merging configuration file ${path.basename(sourcePath)}: ${error instanceof Error ? error.message : String(error)}`);
      return false;
    }
  }
  ...
  static async mergeConfigFiles(
    ...
    for (const [sourcePath, targetPath] of files.entries()) {
      ...
      if (fileType !== ConfigFileType.UNKNOWN) {
        ...
        success = await this.mergeConfigFile(
          sourcePath,
          targetPath,
          {
            interactive,
