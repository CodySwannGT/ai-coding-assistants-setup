  ...
  group('Soak Test Operations', () => {
    ...
    group('Complex Operations', () => {
      ...
      if (__ITER % 5 === 0) { // Every 5th iteration
        const searchResponse = http.get(`${BASE_URL}/api/search?q=test&limit=100`, {
          headers,
...
export function handleSummary(data) {
  ...
  const summary = {
    ...
    performance: {
      ...
      final_avg_response_time: Math.round(metrics.http_req_duration?.values?.avg || 0),
      ...
      degradation_percentage: (metrics.degradation_rate?.values?.avg || 0) * 100,
    },
    resource_usage: {
      requests_per_minute: Math.round((metrics.http_reqs?.values?.count || 0) / (duration / 60)),
